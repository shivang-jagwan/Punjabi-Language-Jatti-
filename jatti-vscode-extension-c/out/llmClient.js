...
exports.JattiLLMClient = void 0;
exports.registerLLMCommands = registerLLMCommands;
 ...
 */
class JattiLLMClient {
    constructor(context, apiUrl = "https://jatti-llm-backend-production.up.railway.app") {
     ...
     */
    async generateCode(prompt, selectedCode) {
        try {
            // Show progress
...
exports.JattiLLMClient = JattiLLMClient;
/**
 * Register LLM commands with VS Code
 */
function registerLLMCommands(context, llmClient) {
    ...
    const generateCommand = vscode.commands.registerCommand("jatti.generateCode", async () => {
        ...
        if (!ready) {
            vscode.window.showErrorMessage("LLM Server not running. Start it with: python scripts/llm_server.py");
            return;
        }
        // Generate code
        ...
        if (!validation.valid) {
            const showResult = await vscode.window.showWarningMessage(`Generated code has issues:\n${validation.errors.join("\n")}`, "Insert Anyway", "Discard");
    ...
    const explainCommand = vscode.commands.registerCommand("jatti.explainCode", async () => {
        ...
        const webview = vscode.window.createWebviewPanel("jattiExplanation", "Jatti Code Explanation", vscode.ViewColumn.Beside, {});
